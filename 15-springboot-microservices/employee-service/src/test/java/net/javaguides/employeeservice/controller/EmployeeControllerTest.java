...
class EmployeeControllerTest {
    ...
    void getEmployee() throws  Exception{
        APIResponseDto apiResponseDto = getValidApiResponseDto();
        given(employeeService.getEmployeeById(any())).willReturn(apiResponseDto);

        mockMvc.perform(get("/api/employees/1").accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.employeeDto.firstName").value("Eliel"));
    }
    ...
    APIResponseDto getValidApiResponseDto(){
        EmployeeDto employeeDto = new EmployeeDto();
        employeeDto.setFirstName("Eliel");
        employeeDto.setLastName("Herrera");
        employeeDto.setEmail("gressheliel@gmail.com");
        ...
        departmentDto.setDepartmentName("Technology Information");
        departmentDto.setDepartmentDescription("Depto de TI");
